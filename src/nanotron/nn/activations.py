...
class XIELU(nn.Module):
    def __init__(self, alpha_p_init=0.8, alpha_n_init=0.8, beta=0.5, eps=1e-6):
        ...
        self.alpha_n = NanotronParameter(torch.log(torch.exp(torch.tensor(alpha_n_init - self.beta)) - 1).unsqueeze(0))
...
# Based on integral of PolyReLU
class XIEP(nn.Module):
    def __init__(self, alpha_p3_init=0.4, alpha_p2_init=0.8, alpha_n_init=0.8, beta=0.5, eps=1e-6):
        super(XIEP, self).__init__()
        self.beta = beta
        self.alpha_p3 = NanotronParameter(torch.log(torch.exp(torch.tensor(alpha_p3_init)) - 1).unsqueeze(0))
        self.alpha_p2 = NanotronParameter(torch.log(torch.exp(torch.tensor(alpha_p2_init)) - 1).unsqueeze(0))
        self.alpha_n = NanotronParameter(torch.log(torch.exp(torch.tensor(alpha_n_init - self.beta)) - 1).unsqueeze(0))
        self.eps = torch.tensor(eps)
    ...
    def forward(self, x):
        ...
        alpha_n = self.beta + F.softplus(self.alpha_n)
        return torch.where(x > 0,
                           alpha_p3 * x * x * x + alpha_p2 * x * x + self.beta * x,
                           alpha_n * torch.expm1(torch.min(x, -self.eps)) - alpha_n * x + self.beta * x)
